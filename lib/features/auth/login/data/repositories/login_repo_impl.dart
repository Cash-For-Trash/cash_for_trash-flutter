...
class LoginRepoImpl implements LoginRepository {
  ...
  Future<Either<String, LoginModel>> login(String email, String password) async {
    ...
    result.fold(
      (error) {},
      (success) async {
        await CacheHelper.saveData(key: ApiKey.accessToken, value: success.data.accessToken);
        await CacheHelper.saveData(key: ApiKey.refreshToken, value: success.data.refreshToken);
        await CacheHelper.saveData(key: ApiKey.firstName, value: success.data.user.first_name);
        await CacheHelper.saveData(key: ApiKey.lastName, value: success.data.user.last_name);
        await CacheHelper.saveData(key: ApiKey.email, value: success.data.user.email);
        await CacheHelper.saveData(key: ApiKey.role, value: success.data.user.role);
        await CacheHelper.saveData(key: ApiKey.user, value: success.data.user.user_id);
      },
    );
